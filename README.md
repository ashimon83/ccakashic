...
- **Waiting-for-you indicator** — Sessions where Claude finished its turn and is awaiting your input get an orange frame and a `⏳ 入力待ち` badge (`🔐 承認待ち` when cmux reports a pending permission prompt). The browser tab title shows the count (`(2) ccakashic`) and the favicon turns orange, so a glance at the tab tells you how many sessions need you — derived from the session jsonl, enriched by cmux's unread notifications
